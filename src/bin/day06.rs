...
fn second(input: &Vec<&str>, max_distance: Option<i32>) -> u32 {
    ...
    for y in miny..=maxy {
        for x in minx..=maxx {
            let distances = points.iter().map(|p| {
                distance(p, &(x, y))
            });
...
fn parse(input: &Vec<&str>) -> Vec<Point> {
    input.iter().map(|input| {
        ...
        (parse_capture(&cap, 1, "line").unwrap(), parse_capture(&cap, 2, "line").unwrap())
    }).collect()
}
...
mod test {
    ...
    #[test]
    fn test2() {
        assert_eq!(second(&vec!["1, 1", "1, 6", "8, 3", "3, 4", "5, 5", "8, 9"], Some(32)), 16);
    }
}
